...
const processImages = () => {
    ...
    if (fs.existsSync(bigDataPath)) {
        ...
        bigData.forEach(p => {
            if (p.province_name)
                tokenize(p.province_name).forEach(t => forbiddenTokens.add(t));
            p.districts.forEach(d => {
                if (d.district_name)
                    tokenize(d.district_name).forEach(t => forbiddenTokens.add(t));
                d.cities.forEach(c => {
                    if (c.city_name)
                        tokenize(c.city_name).forEach(t => forbiddenTokens.add(t));
                    c.traveling_places.forEach(tp => {
                        if (tp.name)
                            tokenize(tp.name).forEach(t => forbiddenTokens.add(t));
                    });
                });
            });
        });
    ...
    if (fs.existsSync(allDataPath)) {
        ...
        allData.forEach(item => {
            if (item.Name)
                tokenize(item.Name).forEach(t => forbiddenTokens.add(t));
        });
    }
    ...
    if (fs.existsSync(allDataPath)) {
        ...
        allData.forEach(item => {
            ...
            if (item.image_urls) {
                ...
                if (item.image_urls.length < originalLen)
                    cleanedCount++;
            }
            // 2. Match
            if (!item.Name)
    ...
    if (fs.existsSync(bigDataPath)) {
        ...
        const updateEntity = (entity) => {
            ...
            if (entity.image_urls) {
                ...
                if (entity.image_urls.length < originalLen)
                    cleanedCount++;
            }
            // 2. Match
